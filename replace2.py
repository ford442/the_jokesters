...
content = re.sub(
    r"import \{\s+runAIExistentialCrisisModeLoop,\s+runSentientSpellcheckerLoop,\s+runSmartThermostatRebellionLoop\s+\} from '\./modes/DreamModes_Tech';",
    "import {\n    runAIExistentialCrisisModeLoop,\n    runSentientSpellcheckerLoop,\n    runSmartThermostatRebellionLoop,\n    runSentientCloudInfrastructureLoop\n} from './modes/DreamModes_Tech';",
    content
)

# Add to Scenario type
content = re.sub(
    r"\| 'galactic_hoa_meeting';",
    "| 'galactic_hoa_meeting' | 'parallel_universe_cable_tv' | 'sentient_cloud_infrastructure' | 'time_traveling_irs_audit';",
    content
)
