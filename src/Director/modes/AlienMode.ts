...
export async function runAlienLoop(scenario: Scenario, ctx: ModeContext) {
    ...
    // 1. Initial Contact
    await chatForAgentWithComedy(ctx, xenolinguist, `(You are a Xenolinguist. You just received a signal from an alien entity (the User). Describe the signal in scientific terms (frequency, modulation). Ask the entity to identify itself using prime numbers.)`, async (s) => await ctx.callbacks.onSpeak(s, xenolinguist, {}));
    ...
    while (ctx.isRunning()) {
        ...
        await chatForAgentWithComedy(ctx, diplomat, diplomatPrompt, async (s) => await ctx.callbacks.onSpeak(s, diplomat, {}));
        ...
        await chatForAgentWithComedy(ctx, exobiologist, bioPrompt, async (s) => await ctx.callbacks.onSpeak(s, exobiologist, {}));
        ...
        await chatForAgentWithComedy(ctx, xenolinguist, linguistPrompt, async (s) => await ctx.callbacks.onSpeak(s, xenolinguist, {}));
    }
}
