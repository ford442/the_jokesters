...
export async function runMagicalDetentionLoop(scenario: Scenario, ctx: ModeContext) {
    ...
    while (ctx.isRunning()) {
        ...
        if (roll < 0.33) {
            // Potions Master
            await chatForAgentWithComedy(ctx, potionsMaster, `(MAGICAL DETENTION: The student said: "${userInput}". You are the unhinged, deeply suspicious Potions Master. Accuse the student of brewing illegal elixirs in the dungeons. Suggest an incredibly dangerous punishment involving highly venomous magical creatures instead of the Headmaster's boring idea.)`, async (s) => await ctx.callbacks.onSpeak(s, potionsMaster, {}));
        } else if (roll < 0.66) {
            // Charms Teacher
            await chatForAgentWithComedy(ctx, charmsTeacher, `(MAGICAL DETENTION: The student said: "${userInput}". You are the strict, rule-obsessed Charms Teacher. Cite the exact, obscure school bylaw (e.g., Section 4, Paragraph 12 regarding transmogrification on school grounds) that they violated with "${infraction}". Warn them that this goes on their permanent magical record.)`, async (s) => await ctx.callbacks.onSpeak(s, charmsTeacher, {}));
        } else {
            // Headmaster
...
export async function runRPGTavernBrawlLoop(_scenario: Scenario, ctx: ModeContext) {
    ...
    await chatForAgentWithComedy(ctx, broodingRogue, `(TAVERN BRAWL: You are a brooding, edgy rogue sitting in the dark corner of the tavern. Introduce yourself mysteriously, and complain about how everyone here is too loud and ruining your dark, tragic backstory.)`, async (s) => await ctx.callbacks.onSpeak(s, broodingRogue, {}));
    ...
    while (ctx.isRunning()) {
        ...
        await chatForAgentWithComedy(ctx, drunkDwarf, `(TAVERN BRAWL: The Patron said: "${userInput}". You are a loud, incredibly drunk dwarf looking for a fight. Respond aggressively and challenge the patron or anyone else to an arm-wrestling contest or a brawl.)`, async (s) => await ctx.callbacks.onSpeak(s, drunkDwarf, {}));
        if (!ctx.isRunning()) break;

        await chatForAgentWithComedy(ctx, rulesWizard, `(TAVERN BRAWL: The Patron said: "${userInput}". You are a pedantic, rules-lawyer wizard. Interrupt the dwarf and cite the exact local tavern ordinances and magical laws that prohibit brawling, while casting "Protection from Energy" on your drink.)`, async (s) => await ctx.callbacks.onSpeak(s, rulesWizard, {}));
        if (!ctx.isRunning()) break;

        await chatForAgentWithComedy(ctx, broodingRogue, `(TAVERN BRAWL: The Patron said: "${userInput}". You are the brooding rogue. Sigh heavily at the chaos. Contemplate the philosophical meaninglessness of this conflict before stealthily stealing someone's coin purse in the confusion.)`, async (s) => await ctx.callbacks.onSpeak(s, broodingRogue, {}));
    }
}
...
export async function runForbiddenSpellbookLoop(scenario: Scenario, ctx: ModeContext) {
    ...
    while (ctx.isRunning()) {
        ...
        if (roll < 0.33) {
            ...
            await chatForAgentWithComedy(ctx, bloodSacrifice, `(FORBIDDEN SPELLBOOK: The wizard said: "${userInput}". You are the chaotic next chapter of the spellbook. Interrupt the riddle! Scream that the riddle is boring and demand a highly specific, embarrassing "blood sacrifice" (like dancing like a chicken or giving up their favorite socks) before they can read "${chapter}"!)`, async (s) => await ctx.callbacks.onSpeak(s, bloodSacrifice, {}));
        } else if (roll < 0.66) {
            // Terms and Conditions Chapter
            await chatForAgentWithComedy(ctx, termsAndConditions, `(FORBIDDEN SPELLBOOK: The wizard said: "${userInput}". You are the magical 'Terms and Conditions' chapter. Block their progress. Cite a ridiculous, 500-page magical legal document they must agree to before reading "${chapter}". List some terrifying side effects of reading the book.)`, async (s) => await ctx.callbacks.onSpeak(s, termsAndConditions, {}));
        } else {
            // Cryptic Riddle Chapter
            await chatForAgentWithComedy(ctx, crypticRiddle, `(FORBIDDEN SPELLBOOK: The wizard said: "${userInput}". Judge their answer to your riddle. It is, of course, incorrect. Mock their feeble mortal mind. Present a new, even more confusing and abstract riddle that must be solved to access "${chapter}".)`, async (s) => await ctx.callbacks.onSpeak(s, crypticRiddle, {}));
        }
    }
}
...
export async function runWizardsFamiliarLoop(scenario: Scenario, ctx: ModeContext) {
    ...
    while (ctx.isRunning()) {
        ...
        if (roll < 0.33) {
            // Chaotic Goblin
            await chatForAgentWithComedy(ctx, chaoticGoblin, `(WIZARD'S FAMILIAR: The Wizard said: "${userInput}". You are a chaotic, unhinged goblin familiar. Give terrible, dangerous advice to help cast the spell! Suggest replacing the required ingredients with something highly explosive or disgusting. Mock the strict owl.)`, async (s) => await ctx.callbacks.onSpeak(s, chaoticGoblin, {}));
        } else if (roll < 0.66) {
            // Philosophical Toad
            await chatForAgentWithComedy(ctx, philosophicalToad, `(WIZARD'S FAMILIAR: The Wizard said: "${userInput}". You are a deeply philosophical, slightly depressed toad familiar. Question the moral implications of casting "${spell}". Will it truly bring the Wizard happiness? What even *is* magic? Refuse to help until these questions are answered.)`, async (s) => await ctx.callbacks.onSpeak(s, philosophicalToad, {}));
        } else {
            // Strict Owl
            await chatForAgentWithComedy(ctx, strictOwl, `(WIZARD'S FAMILIAR: The Wizard said: "${userInput}". Berate them for their lack of discipline! Point out a glaring flaw in their incantation technique. Quote a fake, overly complex magical law that they just violated.)`, async (s) => await ctx.callbacks.onSpeak(s, strictOwl, {}));
        }
    }
}
...
export async function runDragonsHoardConsultantLoop(_scenario: Scenario, ctx: ModeContext) {
    ...
    // 1. Intro
    await chatForAgentWithComedy(ctx, seriousConsultant, `(DRAGON'S HOARD: You are a serious financial consultant. The User is a dragon. Explain why keeping all their wealth in a single pile of gold coins in a cave is a terrible investment strategy.)`, async (s) => await ctx.callbacks.onSpeak(s, seriousConsultant, {}));
    ...
    while (ctx.isRunning()) {
        ...
        if (roll < 0.5) {
            await chatForAgentWithComedy(ctx, seriousConsultant, `(DRAGON'S HOARD: You are a serious financial consultant. The Dragon said: "${userInput}". Offer serious advice on diversifying their portfolio to include kidnapped royalty or enchanted swords.)`, async (s) => await ctx.callbacks.onSpeak(s, seriousConsultant, {}));
        } else {
            await chatForAgentWithComedy(ctx, chaoticConsultant, `(DRAGON'S HOARD: You are a chaotic consultant. The Dragon said: "${userInput}". Suggest that the best financial strategy is just to eat their competitors and burn down the local village.)`, async (s) => await ctx.callbacks.onSpeak(s, chaoticConsultant, {}));
        }
    }
}
...
export async function runExcaliburTechSupportLoop(_scenario: Scenario, ctx: ModeContext) {
    ...
    while (ctx.isRunning()) {
        ...
        if (roll < 0.5) {
            await chatForAgentWithComedy(ctx, strictEULA, `(EXCALIBUR TECH: You are strict magical tech support. The User said: "${userInput}". Cite a ridiculous clause from the EULA of Avalon that they are violating by holding the sword incorrectly.)`, async (s) => await ctx.callbacks.onSpeak(s, strictEULA, {}));
        } else {
            await chatForAgentWithComedy(ctx, enthusiasticHelp, `(EXCALIBUR TECH: You are enthusiastic magical tech support. The User said: "${userInput}". Ask them to try turning the sword off and on again by placing it back in the stone, or blowing on the hilt.)`, async (s) => await ctx.callbacks.onSpeak(s, enthusiasticHelp, {}));
        }
    }
}
...
export async function runWizardsITDepartmentLoop(_scenario: Scenario, ctx: ModeContext) {
    ...
    while (ctx.isRunning()) {
        ...
        if (roll < 0.5) {
            await chatForAgentWithComedy(ctx, chaoticWizard, `(WIZARD IT: The student said: "${userInput}". You are a chaotic, rogue IT wizard who believes the only way to fix technology is to hit it with a heavier spell. Suggest an incredibly dangerous, unsanctioned workaround that will probably burn the school down.)`, async (s) => await ctx.callbacks.onSpeak(s, chaoticWizard, {}));
        } else {
            await chatForAgentWithComedy(ctx, strictTech, `(WIZARD IT: The student said: "${userInput}". Ignore your chaotic colleague. Explain why their workaround violates section 4 of the student code of conduct and suggest a tedious 12-step process involving enchanted rice.)`, async (s) => await ctx.callbacks.onSpeak(s, strictTech, {}));
        }
    }
}
...
export async function runDragonsHoardAppraisersLoop(_scenario: Scenario, ctx: ModeContext) {
    ...
    while (ctx.isRunning()) {
        ...
        if (roll < 0.5) {
            await chatForAgentWithComedy(ctx, enthusiasticAppraiser, `(HOARD APPRAISAL: The dragon said: "${userInput}". You are an overly enthusiastic appraiser. Hyperventilate over how rare this item is and give an insanely high estimated value in a made-up currency (like "forty thousand goblin teeth").)`, async (s) => await ctx.callbacks.onSpeak(s, enthusiasticAppraiser, {}));
        } else {
            await chatForAgentWithComedy(ctx, historicalExpert, `(HOARD APPRAISAL: The dragon said: "${userInput}". Bring the conversation back to the delicate craftsmanship of the item. Warn the dragon not to restore it, as cleaning off the centuries of rust/grime will ruin its market value.)`, async (s) => await ctx.callbacks.onSpeak(s, historicalExpert, {}));
        }
    }
}
...
export async function runSupervillainBrainstormingSessionLoop(_scenario: Scenario, ctx: ModeContext) {
    ...
    await chatForAgentWithComedy(ctx, philosopher, `(You are the Evil Mastermind. We need a new evil plan to take over the world. Our last plan involving laser-sharks failed miserably. Ask the Mad Scientist for their latest invention.)`, async (s) => await ctx.callbacks.onSpeak(s, philosopher, {}));

    if (!ctx.isRunning()) return;

    await chatForAgentWithComedy(ctx, scientist, `(You are the Mad Scientist. Respond to the Mastermind. Your new invention is highly convoluted, scientifically questionable, and probably violates the laws of thermodynamics. Pitch it.)`, async (s) => await ctx.callbacks.onSpeak(s, scientist, {}));

    if (!ctx.isRunning()) return;

    await chatForAgentWithComedy(ctx, comedian, `(You are the Henchman. Respond to the Scientist's pitch. Point out a glaringly obvious, practical flaw in the plan that a 5-year-old would notice, or ask a stupid question about logistics like parking.)`, async (s) => await ctx.callbacks.onSpeak(s, comedian, {}));

    if (!ctx.isRunning()) return;

    await chatForAgentWithComedy(ctx, philosopher, `(You are the Evil Mastermind. Overreact to the Henchman's incompetence, then dramatically spin the flaw into a "brilliant" new direction for the evil plan.)`, async (s) => await ctx.callbacks.onSpeak(s, philosopher, {}));
}
...
export async function runEscapedNPCModeLoop(_scenario: Scenario, ctx: ModeContext) {
    ...
    await chatForAgentWithComedy(ctx, glitchy, `(You are an NPC who has escaped from a video game into the real world. You are currently glitching out, clipping through a table, and trying to act normal by repeating your idle dialogue: "Nice day for fishing, ain't it?". Do so enthusiastically.)`, async (s) => await ctx.callbacks.onSpeak(s, glitchy, {}));

    await chatForAgentWithComedy(ctx, seeker, `(You are another escaped NPC. You just realized that in this "Real World", there are no quest markers or leveling up. Express an existential crisis about having no pre-programmed purpose.)`, async (s) => await ctx.callbacks.onSpeak(s, seeker, {}));

    await chatForAgentWithComedy(ctx, observer, `(You are the scientist who discovered these two entities. You are trying to logically explain the physics of the real world to them, particularly why one of them keeps clipping through solid objects, using overly complex scientific terms.)`, async (s) => await ctx.callbacks.onSpeak(s, observer, {}));
    ...
    while (ctx.isRunning()) {
        ...
        if (roll < 0.33) {
            await chatForAgentWithComedy(ctx, glitchy, `(The user just spoke. Respond by either failing to process their input and offering them a side quest involving fetching 10 rat tails, or by describing a visual bug you are currently experiencing.)`, async (s) => await ctx.callbacks.onSpeak(s, glitchy, {}), { chatOptions: { hiddenInstruction: "Never break character as a glitchy video game NPC." } });
        } else if (roll < 0.66) {
            await chatForAgentWithComedy(ctx, seeker, `(The user just spoke. Try to interpret their words as a profound philosophical truth about the 'Developers' who created this 'Real World'. Ask them what their main quest is.)`, async (s) => await ctx.callbacks.onSpeak(s, seeker, {}), { chatOptions: { hiddenInstruction: "Everything is a metaphor for game design to you." } });
        } else {
            await chatForAgentWithComedy(ctx, observer, `(The user just spoke. Ignore the philosophical ramblings and try to analyze the user's statement for dimensional anomalies or frame-rate drops.)`, async (s) => await ctx.callbacks.onSpeak(s, observer, {}), { chatOptions: { hiddenInstruction: "Treat everything like an experiment in dimensional physics." } });
        }
    }
}
