...
export async function runRapidFireTrivia(scenario: Scenario, ctx: ModeContext) {
  ...
  for (let i = 0; i < questionCount && ctx.isRunning(); i++) {
    ...
    // Ask question
    await chatForAgentWithComedy(ctx, asker, `(You are asking a rapid-fire trivia question about ${topic}. Ask ONE clear, specific question. Make it challenging but fair. Keep it to 1-2 sentences maximum.)`, async (sentence) => await ctx.callbacks.onSpeak(sentence, asker, { steps: RAPID_FIRE_TTS_STEPS }));
    ...
    try {
      ...
      await chatForAgentWithComedy(ctx, judge, `(You just heard the answer: "${userInput}". Judge if it's correct, close, or completely wrong. Be quick and witty about it. 1-2 sentences max.)`, async (sentence) => await ctx.callbacks.onSpeak(sentence, judge, { steps: RAPID_FIRE_TTS_STEPS }));
    } catch {
      totalQuestions++;
...
export async function runRapidFireRoast(scenario: Scenario, ctx: ModeContext) {
  ...
  for (let round = 0; round < roundCount && ctx.isRunning(); round++) {
    for (const comedian of comedians) {
      ...
      await chatForAgentWithComedy(ctx, comedian, `(Rapid-fire roast round ${round + 1}/${roundCount}. Make ONE quick, biting joke about ${roastTarget}. Keep it fast and punchy - 1-2 sentences max. No setup needed.)`, async (sentence) => await ctx.callbacks.onSpeak(sentence, comedian, { steps: RAPID_FIRE_TTS_STEPS }));
    }
  }
...
export async function runRapidFireAssociation(scenario: Scenario, ctx: ModeContext) {
  ...
  for (let turn = 0; turn < turnCount && ctx.isRunning(); turn++) {
    ...
    await chatForAgentWithComedy(ctx, speaker, `(RAPID WORD ASSOCIATION: Previous word was "${currentWord}". Say ONE word that comes to mind. Just the word itself, nothing else. Be fast!)`, async (sentence) => {
...
export async function runRapidFireThisOrThat(scenario: Scenario, ctx: ModeContext) {
  ...
  for (let round = 0; round < Math.min(roundCount, choices.length) && ctx.isRunning(); round++) {
    ...
    // Ask the question
    await chatForAgentWithComedy(ctx, asker, `(RAPID-FIRE CHOICE: Ask the user to choose between "${choice.a}" or "${choice.b}". Be enthusiastic and quick! 1 sentence.)`, async (sentence) => await ctx.callbacks.onSpeak(sentence, asker, { steps: RAPID_FIRE_TTS_STEPS }));
    ...
    await chatForAgentWithComedy(ctx, commenter, `(User chose: "${userInput}". React quickly and witty to their choice between "${choice.a}" or "${choice.b}". 1-2 sentences max, be funny!)`, async (sentence) => await ctx.callbacks.onSpeak(sentence, commenter, { steps: RAPID_FIRE_TTS_STEPS }));
  }
