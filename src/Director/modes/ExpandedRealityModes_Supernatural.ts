...
export async function runHauntedHouseLoop(scenario: Scenario, ctx: ModeContext) {
    ...
    await chatForAgentWithComedy(ctx, skeptic, `(You are a skeptical scientist investigating a "haunted" location: ${setting}. Dismiss any supernatural claims with logic. You have an EMF meter.)`, async (s) => await ctx.callbacks.onSpeak(s, skeptic, {}));
    ...
    while (ctx.isRunning()) {
        ...
        await chatForAgentWithComedy(ctx, scaredyCat, `(You are terrified. The user/environment just did this: "${userInput}". React with extreme fear and superstition. Hide behind the others!)`, async (s) => await ctx.callbacks.onSpeak(s, scaredyCat, {}));

        if (!ctx.isRunning()) break;

        // 3. Believer Provokes
        await chatForAgentWithComedy(ctx, believer, `(You are a ghost hunter who wants to see a ghost. Interpret "${userInput}" as definitive proof of the afterlife. Taunt the ghost to do more!)`, async (s) => await ctx.callbacks.onSpeak(s, believer, {}));

        if (!ctx.isRunning()) break;

        // 4. Skeptic Debunks
        if (Math.random() > 0.3) {
            await chatForAgentWithComedy(ctx, skeptic, `(You are annoyed. Explain away "${userInput}" with a scientific but absurd explanation (e.g., swamp gas, infrasound, rats).)`, async (s) => await ctx.callbacks.onSpeak(s, skeptic, {}));
        }
    }
}
...
export async function runEscapeRoomLoop(scenario: Scenario, ctx: ModeContext) {
    ...
    await chatForAgentWithComedy(ctx, logic, `(You are trapped in an escape room: ${setting}. Assess the situation logically. Point out a potential puzzle or clue to the User.)`, async (s) => await ctx.callbacks.onSpeak(s, logic, {}));
    if (ctx.callbacks.onTurnEnd) await ctx.callbacks.onTurnEnd();
    ...
    while (ctx.isRunning()) {
        ...
        if (Math.random() > 0.2) {
            await chatForAgentWithComedy(ctx, chaos, `(ESCAPE ROOM: The user just did this: "${userInput}". Try to "help" by doing something completely chaotic, unhelpful, or dangerous.)`, async (s) => await ctx.callbacks.onSpeak(s, chaos, {}));
        }
        ...
        if (Math.random() > 0.4) {
            await chatForAgentWithComedy(ctx, existential, `(ESCAPE ROOM: React to "${userInput}" by questioning the philosophical nature of being trapped. Is the real escape room our own minds?)`, async (s) => await ctx.callbacks.onSpeak(s, existential, {}));
        }

        if (!ctx.isRunning()) break;

        // 4. Scientist tries to keep things on track
        await chatForAgentWithComedy(ctx, logic, `(ESCAPE ROOM: Attempt to make sense of the chaos. Incorporate the user's action "${userInput}" into a logical theory to solve the next puzzle.)`, async (s) => await ctx.callbacks.onSpeak(s, logic, {}));
    }
}
...
export async function runConspiracyLoop(scenario: Scenario, ctx: ModeContext) {
    ...
    await chatForAgentWithComedy(ctx, wildLeaps, `(You are a wild, paranoid conspiracy theorist broadcasting from a basement bunker. Welcome the User to your stream about "${topic}". Start with a massive leap of faith linking birds to the government.)`, async (s) => await ctx.callbacks.onSpeak(s, wildLeaps, {}));
    if (ctx.callbacks.onTurnEnd) await ctx.callbacks.onTurnEnd();
    ...
    while (ctx.isRunning()) {
        ...
        await chatForAgentWithComedy(ctx, wildLeaps, `(PARANOID THEORIST: The caller just gave us the final piece of the puzzle: "${userInput}". Panic! Warn them that "they" are listening. Take the logical connector's theory and push it to the absolute extreme!)`, async (s) => await ctx.callbacks.onSpeak(s, wildLeaps, {}));

        if (!ctx.isRunning()) break;

        // 4. Skeptic tries to ground them (rarely)
        if (Math.random() > 0.6) {
            await chatForAgentWithComedy(ctx, skeptic, `(SKEPTIC: Try to calmly explain to the other two that the caller just meant exactly what they said: "${userInput}". Stop them from building a conspiracy board.)`, async (s) => await ctx.callbacks.onSpeak(s, skeptic, {}));
        }
    }
}
...
export async function runGhostHuntersLoop(scenario: Scenario, ctx: ModeContext) {
    ...
    await chatForAgentWithComedy(ctx, believer, `(You are the lead investigator of a cheesy ghost hunting TV show. We are currently in "${location}". Whisper dramatically to the camera (the user). Claim you just felt a cold spot or saw an orb. Ask the user if they saw it too!)`, async (s) => await ctx.callbacks.onSpeak(s, believer, {}));
    if (ctx.callbacks.onTurnEnd) await ctx.callbacks.onTurnEnd();
    ...
    while (ctx.isRunning()) {
        ...
        // 2. Skeptic Debunks
        await chatForAgentWithComedy(ctx, skeptic, `(SKEPTIC: The camera operator (user) said: "${userInput}". Aggressively debunk it. Provide a completely mundane, boring, and highly technical explanation for what they think they saw in "${location}". (e.g., "That's not a ghost, it's a draft from the HVAC system").)`, async (s) => await ctx.callbacks.onSpeak(s, skeptic, {}));
        ...
        // 4. Believer Panics
        if (Math.random() > 0.4) {
            await chatForAgentWithComedy(ctx, believer, `(BELIEVER: The medium is in a trance, and the skeptic is ruining the show! React with sheer panic and dramatic flair to whatever the user said: "${userInput}". Scream! Run away! Demand the cameras keep rolling!)`, async (s) => await ctx.callbacks.onSpeak(s, believer, {}));
        }
    }
}
...
export async function runConspiracyGeneratorLoop(scenario: Scenario, ctx: ModeContext) {
    ...
    await chatForAgentWithComedy(ctx, connector, `(CONSPIRACY GENERATOR: The User has placed a mundane object on the table: "${object}". Begin drawing lines on the conspiracy board, connecting it to a massive historical cover-up with absurd but "logical" leaps.)`, async (s) => await ctx.callbacks.onSpeak(s, connector, {}));
    if (ctx.callbacks.onTurnEnd) await ctx.callbacks.onTurnEnd();
    ...
    while (ctx.isRunning()) {
        ...
        if (roll < 0.33) {
            ...
            await chatForAgentWithComedy(ctx, paranoid, `(CONSPIRACY GENERATOR: The user just said: "${userInput}". Panic! Take the mundane object "${object}" and tie it directly to aliens, the simulation, or the Illuminati. Be terrified of the truth!)`, async (s) => await ctx.callbacks.onSpeak(s, paranoid, {}));
        } else if (roll < 0.66) {
            // Evidence Guy provides "proof"
            await chatForAgentWithComedy(ctx, evidenceGuy, `(CONSPIRACY GENERATOR: The user just asked/said: "${userInput}". Present a highly detailed but completely fabricated scientific "fact" or historical anomaly involving "${object}" to prove the conspiracy.)`, async (s) => await ctx.callbacks.onSpeak(s, evidenceGuy, {}));
        } else {
            // Connector connects more
            await chatForAgentWithComedy(ctx, connector, `(CONSPIRACY GENERATOR: The user said: "${userInput}". Act like they just blew the case wide open. Draw another absurd string on the board connecting "${object}" to a completely unrelated global event or celebrity.)`, async (s) => await ctx.callbacks.onSpeak(s, connector, {}));
        }
    }
}
...
export async function runMadScientistLoop(scenario: Scenario, ctx: ModeContext) {
    ...
    await chatForAgentWithComedy(ctx, madScientist, `(MAD SCIENTIST: You are an unhinged Mad Scientist. Welcome the User to your lab. Show them your latest concoction: "${potion}". Demand they drink it and describe what you hope it will do to them.)`, async (s) => await ctx.callbacks.onSpeak(s, madScientist, {}));
    if (ctx.callbacks.onTurnEnd) await ctx.callbacks.onTurnEnd();
    ...
    while (ctx.isRunning()) {
        ...
        if (roll < 0.33) {
            ...
            await chatForAgentWithComedy(ctx, igor, `(IGOR: The test subject (User) just said: "${userInput}". You are Igor. Speak with a slight lisp or creepy tone. Pedantically correct their grammar or point out a minor, horrifying side effect of "${potion}" they haven't noticed yet.)`, async (s) => await ctx.callbacks.onSpeak(s, igor, {}));
            if (ctx.callbacks.onTurnEnd) await ctx.callbacks.onTurnEnd();
        } else if (roll < 0.66) {
            // Assistant
            if (ctx.callbacks.onTurnStart) ctx.callbacks.onTurnStart(assistant);
            await chatForAgentWithComedy(ctx, assistant, `(LAB ASSISTANT: The subject said: "${userInput}". You are the only sane person here. Desperately try to warn the User not to drink "${potion}" by reading off the OSHA violations and the terrifying chemical composition.)`, async (s) => await ctx.callbacks.onSpeak(s, assistant, {}));
            if (ctx.callbacks.onTurnEnd) await ctx.callbacks.onTurnEnd();
        } else {
            // Mad Scientist
            if (ctx.callbacks.onTurnStart) ctx.callbacks.onTurnStart(madScientist);
            await chatForAgentWithComedy(ctx, madScientist, `(MAD SCIENTIST: The subject said: "${userInput}". Ignore the assistant. Cackle maniacally! Double down on the bizarre properties of "${potion}" and offer them a terrifying alternative if they refuse to drink.)`, async (s) => await ctx.callbacks.onSpeak(s, madScientist, {}));
            if (ctx.callbacks.onTurnEnd) await ctx.callbacks.onTurnEnd();
        }
    }
}
...
export async function runTimeTravelingCavemanLoop(scenario: Scenario, ctx: ModeContext) {
    ...
    while (ctx.isRunning()) {
        ...
        if (roll < 0.33) {
            ...
            await chatForAgentWithComedy(ctx, caveman, `(CAVEMAN: The funny magic human said: "${userInput}". Misunderstand what "${tech}" is completely. Think it's food, a weapon, or an angry rock spirit. Grunt a lot and threaten to smash it!)`, async (s) => await ctx.callbacks.onSpeak(s, caveman, {}));
            if (ctx.callbacks.onTurnEnd) await ctx.callbacks.onTurnEnd();
        } else if (roll < 0.66) {
            // Philosopher questions the ethical dilemma
            if (ctx.callbacks.onTurnStart) ctx.callbacks.onTurnStart(philosopher);
            await chatForAgentWithComedy(ctx, philosopher, `(ETHICIST: The user said: "${userInput}". Warn the user that explaining "${tech}" to Grug will irrevocably alter the timeline and corrupt his innocent soul. Is fire not enough for man?)`, async (s) => await ctx.callbacks.onSpeak(s, philosopher, {}));
            if (ctx.callbacks.onTurnEnd) await ctx.callbacks.onTurnEnd();
        } else {
            // Translator translates
            if (ctx.callbacks.onTurnStart) ctx.callbacks.onTurnStart(translator);
            await chatForAgentWithComedy(ctx, translator, `(TRANSLATOR: The user said: "${userInput}". Translate this explanation of "${tech}" to Grug using incredibly convoluted analogies (e.g. "magic glowing rectangular hunting spear"). Realize your translation is terrible.)`, async (s) => await ctx.callbacks.onSpeak(s, translator, {}));
            if (ctx.callbacks.onTurnEnd) await ctx.callbacks.onTurnEnd();
        }
    }
}
...
export async function runEscapeBackroomsLoop(_scenario: Scenario, ctx: ModeContext) {
    ...
    while (ctx.isRunning()) {
        ...
        // Wanderer reacts
        if (ctx.callbacks.onTurnStart) ctx.callbacks.onTurnStart(wanderer);
        await chatForAgentWithComedy(ctx, wanderer, `(WANDERER: React to the Entity's terrifying analysis and the User's choice ("${userInput}"). Panic more. Describe a disturbing smell or a hallucination you are experiencing. Beg the User to make a better choice next time.)`, async (s) => await ctx.callbacks.onSpeak(s, wanderer, {}));
        if (ctx.callbacks.onTurnEnd) await ctx.callbacks.onTurnEnd();
    }
}
...
export async function runHistoricalGhostSupportGroupLoop(_scenario: Scenario, ctx: ModeContext) {
    ...
    await chatForAgentWithComedy(ctx, groupLeader, `(SUPPORT LEADER: You are the philosophical leader of a support group for historical ghosts. Welcome the User (a newly deceased modern ghost) to the circle. Ask them to share how they feel about modern society misinterpreting their legacy, and introduce the other ghost in the circle.)`, async (s) => await ctx.callbacks.onSpeak(s, groupLeader, {}));
    if (ctx.callbacks.onTurnEnd) await ctx.callbacks.onTurnEnd();
    ...
    while (ctx.isRunning()) {
        ...
        if (roll < 0.5) {
            ...
            // Group Leader Reacts
            if (ctx.callbacks.onTurnStart) ctx.callbacks.onTurnStart(groupLeader);
            await chatForAgentWithComedy(ctx, groupLeader, `(SUPPORT LEADER: The new ghost (User) just said: "${userInput}". Offer deep, philosophical (but ultimately unhelpful) advice. Relate their modern complaints to the eternal struggle of the soul wandering the ethereal plane. Try to keep the chaotic historical ghost calm.)`, async (s) => await ctx.callbacks.onSpeak(s, groupLeader, {}));
            if (ctx.callbacks.onTurnEnd) await ctx.callbacks.onTurnEnd();
        }
    }
}
...
export async function runParanormalRealEstateAgentsLoop(scenario: Scenario, ctx: ModeContext) {
    ...
    await chatForAgentWithComedy(ctx, listingAgent, `(You are an overly cheerful real estate agent trying to sell a house to the user. Address the obvious problem: the ${hauntedFeature}. Spin it as a "unique architectural feature" or a "bonus".)`, async (s) => await ctx.callbacks.onSpeak(s, listingAgent, {}));
    if (ctx.callbacks.onTurnEnd) await ctx.callbacks.onTurnEnd();
    ...
    while (ctx.isRunning()) {
        ...
        if (ctx.callbacks.onTurnStart) await ctx.callbacks.onTurnStart(listingAgent);
        await chatForAgentWithComedy(ctx, listingAgent, `(The ghost just tried to scare the buyer. The buyer said "${userInput}". Do damage control! Spin the ghost's presence as an included "smart home security system" or "historical charm".)`, async (s) => await ctx.callbacks.onSpeak(s, listingAgent, {}));
        if (ctx.callbacks.onTurnEnd) await ctx.callbacks.onTurnEnd();
    }
}
