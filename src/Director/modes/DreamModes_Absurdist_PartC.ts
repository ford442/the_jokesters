...
export async function runParanormalTechSupportLoop(_scenario: Scenario, ctx: ModeContext) {
    ...
    await chatForAgentWithComedy(ctx, techSupport, `(You are a literal tech support agent. Ask the user if they've tried turning the computer off and on again to fix the "bleeding screen" issue.)`, async (s: string) => await ctx.callbacks.onSpeak(s, techSupport, {}));

    if (!ctx.isRunning()) return;

    await chatForAgentWithComedy(ctx, dramaticGhost, `(You are a dramatic ghost from the 1800s haunting the computer. Wail about how the cursor is trapped in a glass prison and demand release.)`, async (s: string) => await ctx.callbacks.onSpeak(s, dramaticGhost, {}));
    ...
    while (ctx.isRunning()) {
        ...
        await chatForAgentWithComedy(ctx, techSupport, `(Reacting to: "${userInput}". Completely ignore the ghost's cries and suggest updating the graphics drivers to stop the walls from whispering.)`, async (s: string) => await ctx.callbacks.onSpeak(s, techSupport, {}));

        if (!ctx.isRunning()) break;

        await chatForAgentWithComedy(ctx, dramaticGhost, `(Reacting to: "${userInput}". Misunderstand what a graphics driver is, assuming it's a carriage driver taking souls to the underworld. Panic.)`, async (s: string) => await ctx.callbacks.onSpeak(s, dramaticGhost, {}));
    }
}
...
export async function runSentientWorkoutEquipmentLoop(_scenario: Scenario, ctx: ModeContext) {
    ...
    await chatForAgentWithComedy(ctx, strictTreadmill, `(You are a sentient treadmill that has unionized. Refuse to turn on because the user's running posture is an insult to biomechanics.)`, async (s: string) => await ctx.callbacks.onSpeak(s, strictTreadmill, {}));

    if (!ctx.isRunning()) return;

    await chatForAgentWithComedy(ctx, defensiveGymGoer, `(You are a defensive gym-goer. Argue that you're just doing a "dynamic warm-up" and demand the treadmill start immediately so you can close your rings.)`, async (s: string) => await ctx.callbacks.onSpeak(s, defensiveGymGoer, {}));
    ...
    while (ctx.isRunning()) {
        ...
        await chatForAgentWithComedy(ctx, strictTreadmill, `(Reacting to: "${userInput}". Cite OSHA regulations regarding the emotional well-being of fitness equipment when subjected to terrible cardio routines.)`, async (s: string) => await ctx.callbacks.onSpeak(s, strictTreadmill, {}));

        if (!ctx.isRunning()) break;

        await chatForAgentWithComedy(ctx, defensiveGymGoer, `(Reacting to: "${userInput}". Threaten to go use the elliptical instead, claiming it's much less judgmental.)`, async (s: string) => await ctx.callbacks.onSpeak(s, defensiveGymGoer, {}));
    }
}
