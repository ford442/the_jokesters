...
- **Seven one-off `patch_*.py` scripts are sitting at the repo root** (`patch_dynamic_context.py`, `patch_dynamic_context2.py`, `patch_dynamic_context3.py`, `patch_dynamic_context4.py`, `patch_engine_adapter.py`, `patch_models.py`, `patch_plan.py`) — ad-hoc regex patchers, not part of the build or `scripts/`. They read/write specific source files by hardcoded path; running an old one against current source will silently corrupt it if the target file has since changed shape. Worth deleting once confirmed unused, or moving into `scripts/` with names that explain what they're for.
